...
hutToKit (Add a b) = add (hutToKit a) (hutToKit b)
...
type ListF a = K ()
           :+: (K a :*: I) -- replace this right-hand side
...
append xs ys = dataFold appendAlg xs where
  ...
  appendAlg (L (K ())) = ys
  appendAlg (R ((K x) :&: I xs)) = cons x xs
{-Q4-end-----------------------------------------------------}
...
insertIntoList a = dataRec insAlg
    where
  -- insAlg :: ListF a (List a, List a) -> List a
        ...
        insAlg (R ((K x) :&: I (argument, result))) 
            | a < x = cons a (cons x argument)
            | otherwise = cons x result
...
insertionSort = dataFold insSortAlg where
  ...
  insSortAlg (L (K ())) = nil
  insSortAlg (R ((K x) :&: I xs)) = insertIntoList x xs
{-Q5-end-----------------------------------------------------}
...
flatten = dataFold flatAlg where
  ...
  flatAlg (L (K ())) = nil
  flatAlg (R ((I l) :&: (K x) :&: (I r))) = append l (cons x r)
...
insertIntoTree a = dataRec ins where
    ins (L (K ())) = node leaf a leaf
    ins (R ((I (l, l')) :&: (K x) :&: (I (r, r'))))
        | a < x = node l' x r
...
makeTree = dataFold alg where
    alg (L (K ())) = leaf
    alg (R (K x :&: I xs)) = insertIntoTree x xs
...
instance (Listable (f x), Listable (g x)) => Listable ((f :+: g) x) where
  allElements = [L x | x <- allElements] ++ [R x | x <- allElements]

instance (Listable (f x), Listable (g x)) => Listable ((f :*: g) x) where
  allElements = [x :&: y | x <- allElements, y <- allElements]
...
newtype Undefined a x = Undefined (Undefined a x) -- I'll use this as a placeholder
instance Functor (Undefined a) where
